...
diasOcorrencias: function ( mes, ano,dep,siape,ocorrencia,callback) {
	...
	var sql  = "SELECT day(c1.data) as dia   from   (SELECT      f.departamento_id as departamento, f.id, f.n_folha as siape, f.nome,  case when b.entrada1 like  '*%' then b.entrada1 when b.entrada2 like  '*%' then b.entrada2 ELSE  NULL end as  entrada2, b.data,case when entrada1 LIKE '*%' then entrada1 when saida1 LIKE '*%' then saida1 when entrada2 LIKE '*%' then entrada2 when entrada3 LIKE '*%' then entrada3 when saida3 LIKE '*%' then saida3 else null end as ocorrencia from funcionarios f INNER JOIN batidas b on f.id = b.funcionario_id where (b.data between '"+ano+str_mes+"01' and '"+ano+str_mes+days+"' ) and f.departamento_id = "+dep+" and f.n_folha  ='"+siape+"')as c1   JOIN justificativas j on c1.entrada2 = j.nome   where not c1.ocorrencia is null and c1.ocorrencia = '"+ocorrencia+"' order by dia";
